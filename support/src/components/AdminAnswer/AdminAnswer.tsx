...
const MOCK_ORDER_DETAILS: Record<number, SupportOrderFull> = {
    8643352: {
        id: 8643352,
        category_name: 'Технические сбои',
        status: 'sent',
        created_at: '2026-03-12T14:27:00+03:00',
        description: 'Не открывается раздел избранного, страница зависает после авторизации.',
        images: [
            {
                img_url: 'https://picsum.photos/id/1011/640/420',
                order: 0,
            },
            {
                img_url: 'https://picsum.photos/id/1025/640/420',
                order: 1,
            },
        ],
    },
...
export function AdminAnswer({ id }: AdminAnswerProps) {
    ...
    useEffect(() => {
        const loadOrder = async () => {
            ...
            try {
                setLoading(true);
                setError(null);
                const response = await apiService.get(`/support/orders/${orderId}`);
                const loaded = (response?.order || response) as SupportOrderFull;
                setOrder(loaded || MOCK_ORDER_DETAILS[orderId] || null);
            } catch (err: unknown) {
                const fallback = MOCK_ORDER_DETAILS[orderId] || null;
                if (fallback) {
                    setOrder(fallback);
                    setError(null);
                    return;
    ...
    const handleSendAnswer = async () => {
        ...
        try {
            ...
            await apiService.post(`/support/orders/${order.id}/answer`, formData, {
