...
export function PosterParams({ poster }: PosterParamsProps) {
  return (
    <article className={`${shared.card} ${styles.card}`}>
      <h2 className={`${shared.title} ${styles.title}`}>Характеристики</h2>
      <div className={styles.params}>
        ...
        <div className={styles.param}><span className={styles.paramLabel}>Тип жилья</span><span className={styles.paramValue}>{poster.category.name}</span></div>
