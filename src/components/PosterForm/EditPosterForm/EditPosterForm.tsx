...
function ApartmentDetailsToInitialPosterForm(poster: ApartmentDetails): CreatePosterFormData {
  const images = poster.images.map((img) => ({
...
export function EditPosterForm({ poster }: EditPosterFormProps) {
  ...
  useEffect(() => {
    const init = ApartmentDetailsToInitialPosterForm(poster);
    try {
      Object.assign(formDraft, init);
    } catch (err) {
      // ignore
    }
    setForm(init);
  ...
  useEffect(() => {
    try {
      (window as any).__INITIAL_OSM_COORDS__ = poster.building_geo
        ? { lat: poster.building_geo.lat, lon: poster.building_geo.lon }
        : null;
    } catch (err) {
    }
    return () => {
      try {
        delete (window as any).__INITIAL_OSM_COORDS__;
      } catch (err) {
      }
    };
  }, [poster.building_geo]);
